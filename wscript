...
def build(ctx):
  if False and hint is not None:
    try:
      hint([node.abspath() for node in ctx.path.ant_glob("src/**/*.js")],
        _tty_out=False) # no tty because there are none in cloudpebble sandbox
    except ErrorReturnCode_2 as e:
      ctx.fatal("\nJavaScript linting failed:\n" + e.stdout)

  # Concatenate all JS files (but not recursively) if any exist
